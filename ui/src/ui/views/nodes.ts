    ...
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ...
        ${pending.length === 0 && paired.length === 0
          ? html`<div class="muted">暂无配对设备</div>`
          : nothing}
      </div>
    </section>
  `;
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            使用 <span class="mono">exec host=node</span> 时将智能体固定到特定节点
          </div>
        </div>
        <button
          class="btn"
      ...
      ${state.formMode === "raw"
        ? html`<div class="callout warn" style="margin-top: 12px;">
            切换配置标签到<strong>表单</strong>模式以在此处编辑绑定。
          </div>`
        : nothing}
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      ...
      ${state.target === "node" && !hasNodes
        ? html`<div class="muted">暂无节点发布执行审批。</div>`
        : nothing}
    </div>
  `;
}
