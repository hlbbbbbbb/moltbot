...
import { convertMarkdownForWeChat } from "./format.js";
...
async function sendReplyPayload(params: {
  ...
  // 发送文本（转换 Markdown 格式为微信友好格式）
  if (payload.text) {
    // 将 Markdown 转换为微信友好的纯文本格式
    const formattedText = convertMarkdownForWeChat(payload.text);
    log.info(
      `发送文本回复 to=${toUser} length=${formattedText.length} (原始: ${payload.text.length})`,
    );
...
async function processKfMessage(params: {
  ...
  try {
    ...
    const ctx: MsgContext = {
      ...
      OriginatingTo: `kf:${openKfid}:${externalUserId}`,
      // 启用命令处理
